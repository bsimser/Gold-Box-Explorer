...
namespace GoldBoxExplorer.Lib.Plugins.DaxEcl.EclDump
{
    partial class EclDump
    {
        ...
        internal string CMD_CompareAnd()
        {
        ...
        internal string CMD_ClearMonsters()
        {
            ecl_offset++;
        ...
        internal string CMD_Protection()
        {
            EclOpp[] opps = LoadEclOpps(1);
        ...
        internal string CMD_InputString()
        {
            EclOpp[] opps = LoadEclOpps(2);
        ...
        internal string CMD_InputNumber()
        {
            EclOpp[] opps = LoadEclOpps(2);
        ...
        internal string CMD_SetupMonster()
        {
            EclOpp[] opps = LoadEclOpps(3);
        ...
        internal string CMD_LoadCharacter()
        {
            EclOpp[] opps = LoadEclOpps(1);
        ...
        internal string CMD_LoadPieces()
        {
            EclOpp[] opps = LoadEclOpps(3);
        ...
        internal string CMD_Picture() /* sub_26873 */
        {
            EclOpp[] opps = LoadEclOpps(1);
        ...
        internal string CMD_VertMenu()
        {
            ...
            for (int i = 0; i < menuCount; i++)
            {
                ...
                menutext.Append("'");
                if (i + 1 >= stringTable.Length)
                {
                    menutext.Append("<INVALID STRING TABLE REFERENCE>");
                }
                else
                {
        ...
        internal string CMD_HorizontalMenu()
        {
            EclOpp[] opps = LoadEclOpps(2);
        ...
        internal string CMD_LoadMonster()
        {
            EclOpp[] opps = LoadEclOpps(3);
        ...
        internal string CMD_DestroyItems()
        {
            EclOpp[] opps = LoadEclOpps(1);
        ...
        internal string CMD_FindSpecial()
        {
            EclOpp[] opps = LoadEclOpps(1);
        ...
        internal string CMD_ClearBox()
        {
            ecl_offset++;
            return "";
        }

        internal string CMD_Delay()
        {
            ecl_offset++;
            return "";
        }

        internal string CMD_EclClock()
        {
            EclOpp[] opps = LoadEclOpps(2);
        ...
        internal string CMD_Damage() /* sub_28958 */
        {
        ...
        internal string CMD_PrintReturn()
        {
            ecl_offset++;
            return "";
        }

        internal string CMD_NewECL()
        {
            EclOpp[] opps = LoadEclOpps(1);
        ...
        // also the on gosub command
        internal string CMD_OnGoto()
        {
            EclOpp[] opps = LoadEclOpps(2);
            ...
            // check if this is the on goto command that handles the map events, if so, we'll later annotate the subroutines it calls as event 1, event 2 etc
            if (var_2 > 4)
                {
                    if (eventCount < var_2 && !var_1.Contains("invalid"))
                    {
                        eventCount = var_2;
                        _largestOnGotoOpps = opps;
                    }
        ...
        internal string CMD_PartyStrength()
        {
            EclOpp[] opps = LoadEclOpps(1);
        ...
        internal string CMD_EncounterMenu()
        {  
        ...
        internal string CMD_CheckParty()
        {
            EclOpp[] opps = LoadEclOpps(6);
            int var_2 = 0;
            if (opps[0].Code == 1)

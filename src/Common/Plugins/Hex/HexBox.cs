...
namespace GoldBoxExplorer.Lib.Plugins.Hex
{
    public partial class HexBox : Control
    {
        ...
        private void UpdateScrollSize()
        {
            ...
            {
                var scrollmax = (long) Math.Ceiling((_byteProvider.Length + 1)/(double) _iHexMaxHBytes - _iHexMaxVBytes);
        ...
        private int ToScrollPos(long value)
        {
            ...
            var res = (int) Math.Floor(max/(double) 100*valperc);
        ...
        public void ScrollByteIntoView(long index)
        {
            ...
            if (index < _startByte)
            {
                var line = (long) Math.Floor(index/(double) _iHexMaxHBytes);
        ...
        private void CreateCaret()
        {
            ...
            var caretWidth = (int) _charSize.Width;
        ...
        private BytePositionInfo GetHexBytePositionInfo(Point p)
        {
            ...
            return bytePos < 0 ? new BytePositionInfo(0, 0) : new BytePositionInfo(bytePos, byteCharaterPos);
        }
        ...
        private BytePositionInfo GetStringBytePositionInfo(Point p)
        {
            ...
            var bytePos = Math.Min(_byteProvider.Length,
                                    _startByte + (_iHexMaxHBytes*(iY + 1) - _iHexMaxHBytes) + hPos - 1);
            const int byteCharacterPos = 0;

            return bytePos < 0 ? new BytePositionInfo(0, 0) : new BytePositionInfo(bytePos, byteCharacterPos);
        }
        ...
        public void Copy()
        {
            ...
            string sBuffer = Encoding.ASCII.GetString(buffer, 0, buffer.Length);
        ...
        private void PaintColumnSeparator(Graphics g)
        {
            for (int col = GroupSize; col < _iHexMaxHBytes; col += GroupSize)
            {
                ...
                if (StringViewVisible)
                {
                    ...
                    headerPointF.X -= 2;
                    g.DrawLine(pen, new PointF(byteStringPointF.X, byteStringPointF.Y),
                               new PointF(byteStringPointF.X, byteStringPointF.Y + _recHex.Height));
                }
            }
        }
        ...
        private void PaintHex(Graphics g, long startByte, long endByte)
        {
            ...
            bool isKeyInterpreterActive = _keyInterpreter == null ||
        ...
        {
            string sB = b.ToString(_hexStringFormat, Thread.CurrentThread.CurrentCulture);
            if (sB.Length == 1)
                sB = "0" + sB;
        ...
        private void PaintHexAndStringView(Graphics g, long startByte, long endByte)
        {
            ...
            bool isKeyInterpreterActive = _keyInterpreter == null ||
                                          _keyInterpreter.GetType() == typeof (KeyInterpreter);
            bool isStringKeyInterpreterActive = _keyInterpreter != null &&
        ...
        {
            var row = (int) Math.Floor(byteIndex/(double) _iHexMaxHBytes);
        ...
        private byte[] ConvertHexToBytes(string hex)
        {
            ...
            for (int i = 0; i < hexArray.Length; i++)
            {
                ...
                byte b;
                bool isByte = ConvertHexToByte(hexValue, out b);
                if (!isByte)
                    return null;
                byteArray[i] = b;
            }
        ...
        {
            bool isByte = byte.TryParse(hex, NumberStyles.HexNumber, Thread.CurrentThread.CurrentCulture, out b);
            return isByte;
        }
